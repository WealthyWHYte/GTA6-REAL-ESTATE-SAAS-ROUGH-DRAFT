...
function evaluateResearchQuality(property, research) {
  const scores = {
    comps_accuracy: 0,
    arv_calculation: 0,
    market_context: 0,
  ...
  const overallScore = Math.round(scores.comps_accuracy * 0.25 + scores.arv_calculation * 0.25 + scores.market_context * 0.15 + scores.data_completeness * 0.20 + scores.actionability * 0.15);
  // Generate notes
...
function recommendAdditionalData(property, evaluation) {
  const recommendations = [];
  if (evaluation.criteria_scores.comps_accuracy < 70) {
    recommendations.push(`Pull sold properties in ${property.city}, ${property.state} from last 6 months within 1-mile radius`);
    recommendations.push(`Find off-market properties with similar specs (${property.bedrooms}bd/${property.bathrooms}ba)`);
  }
  if (evaluation.criteria_scores.arv_calculation < 70) {
    recommendations.push('Get MLS comparables with renovation details');
    recommendations.push('Find tax assessor values for subject property');
  }
  if (evaluation.criteria_scores.market_context < 60) {
    recommendations.push(`Research ${property.city} market trends - days on market, price per sqft trends`);
    recommendations.push('Find neighborhood amenities and development plans');
  }
  if (evaluation.criteria_scores.data_completeness < 60) {
    recommendations.push('Rental comparables in the area');
    recommendations.push('Property tax history and assessed values');
  }
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: researchResult, error: researchError } = await supabase.from('market_research_results').insert({
    ...
    const { data: evaluationRecord, error: evalError } = await supabase.from('research_evaluations').insert({
      property_id: propertyId,
