  ...
  try {
    ...
    if (apifyToken) {
      try {
        ...
        console.error('Zillow failed:', e.message);
      }
      try {
        ...
        console.error('Redfin failed:', e.message);
      }
    }
    ...
    await supabase.from('properties').update({
      ...
      market_research: researchResults,
      pipeline_status: 'underwriting',
      updated_at: new Date().toISOString()
    }).eq('property_id', property_id);
    await supabase.functions.invoke('underwrite-property', {
      body: {
        property_id,
        account_id
      }
...
async function researchZillow(property, apifyToken) {
  const searchQuery = `${property.address} ${property.city} ${property.state} ${property.zip}`;
  const response = await fetch('https://api.apify.com/v2/acts/apify~zillow-scraper/run', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      startUrls: [
        {
          url: `https://www.zillow.com/homedetails/${searchQuery}`
        }
      ],
      proxy: {
        useApifyProxy: true
...
async function researchRedfin(property, apifyToken) {
  const searchQuery = `${property.address} ${property.city} ${property.state}`;
  const response = await fetch('https://api.apify.com/v2/acts/apify~redfin-scraper/run', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${apifyToken}`,
      'Content-Type': 'application/json'
    ...
    body: JSON.stringify({
      startUrls: [
        {
          url: `https://www.redfin.com/${searchQuery}`
        }
      ],
      proxy: {
        useApifyProxy: true
...
async function waitForApifyResult(token, runId, maxAttempts = 30) {
  for(let i = 0; i < maxAttempts; i++){
    ...
    if (data.data.status === 'SUCCEEDED') {
      const items = await (await fetch(`https://api.apify.com/v2/datasets/${data.data.defaultDatasetId}/items`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })).json();
...
function calculateCashOnCash(price, arv, rent) {
